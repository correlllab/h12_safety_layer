'''Dangerous debug script that sends random joint positions for 1 second'''
...
def _print_danger_warnings(topic: str) -> None:
    ...
    print('!!! RANDOM COMMANDS MAY CAUSE LARGE OR UNEXPECTED MOTION         !!!')
...
def _sample_random_q() -> np.ndarray:
    lows = np.asarray([float(lim['low']) for lim in URDF_POSITION_LIMITS], dtype=np.float32)
    highs = np.asarray([float(lim['high']) for lim in URDF_POSITION_LIMITS], dtype=np.float32)
    return np.random.uniform(lows, highs).astype(np.float32)


def _fill_random_command(cmd: LowCmd_, q_random: np.ndarray) -> None:
    for i in range(MOTOR_COUNT):
        motor = cmd.motor_cmd[i]
        motor.mode = 1
        motor.q = float(q_random[i])
        motor.dq = 0.0
        motor.tau = 0.0
...
def _publish_for_one_second(topic: str, domain_id: int, interface: str | None, hz: float) -> None:
    ...
    while time.time() < end_time:
        q_random = _sample_random_q()
        _fill_random_command(cmd, q_random)
        cmd.crc = crc.Crc(cmd)
        publisher.Write(cmd)
        sent += 1
        time.sleep(dt)

    print(f'Published {sent} random-position commands to {topic}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Dangerous debug: send random positions for 1 second')
